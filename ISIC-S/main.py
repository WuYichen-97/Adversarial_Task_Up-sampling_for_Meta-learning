...
def construct_task_pool(N,task_pool,MB_feats):
    MB_feats = MB_feats.reshape(2,3,84,84)
    temp0 = np.random.beta(3,5,size=2)
    temp0.sort()
    temp = torch.tensor(temp0[0]).cuda()
    temp1 = torch.tensor(temp0[1]).cuda()
    coarse0 = torch.zeros(2*N,2,3,84,84)
    for i in range(N):
        lamda = temp
    ...
    coarse1 = torch.zeros(2*N,2,3,84,84)
    for i in range(N):
        lamda = temp
    ...
    return coarse0,coarse1,MB_feats
...
def select_task_pool(sinkhorn,task_pool,coarse0,coarse1,MB,N):
    # task_pool #(16,5,3,84,84)  
    # coarse #(32,3,84,84)
    class_ori = torch.mean(task_pool.reshape(N,2,3,84,84),dim=0)   # (5,3,84,84)
    ...
    gt = torch.cat((gt0,gt1),dim=0) #(3,5,48,3,84,84)
    sel_gt = torch.zeros(2,2*N,3,84,84) #(5,48,3,84,84)
    for i in range(2):
...
def train(args, maml, optimiser):
    ...
    isic = ISIC('../', mode='train', n_way=args.num_classes, k_shot=args.update_batch_size, k_query=args.update_batch_size_eval,
                batchsz=args.metatrain_iterations*args.meta_batch_size, resize=84)
    
    dataloader = data.DataLoader(isic, args.meta_batch_size, shuffle=False)        
        ...
        for step, (x_spt, y_spt, x_qry, y_qry, MB_x) in enumerate(dataloader):
            ...
            for i in range(args.meta_batch_size):
                if args.update_batch_size ==1: 
                    ...
                    # supp_temp = supp.reshape(2,5,3,84,84).permute(1,0,2,3,4) # (5,2,3,84,84)
                    query_temp = query.reshape(2,15,3,84,84).permute(1,0,2,3,4) #(15,2,3,84,84)
                    task_ori_pool = torch.cat((supp.unsqueeze(0),query_temp),dim=0) #(20,2,3,84,84) 
                    coarse0, coarse1, MB_feats = construct_task_pool(16, task_ori_pool,MB)  
                                    
                    if random.random() <= 0.8:  # Augmentation Ratio 
                        partial_input = task_ori_pool.reshape(32,3,84,84).cuda()
                        gt = select_task_pool(sinkhorn,task_ori_pool,coarse0,coarse1,MB_feats,16)
                    ...
                    query_temp = query.reshape(2,15,3,84,84).permute(1,0,2,3,4) #(15,2,3,84,84)
                    task_ori_pool = torch.cat((supp.unsqueeze(0),query_temp),dim=0) #(20,2,3,84,84) 
                    coarse0, coarse1, MB_feats = construct_task_pool(20, task_ori_pool,MB)  
                                    
                    if random.random() <= 0.8:  # Augmentation Ratio 
                        partial_input = task_ori_pool.reshape(32,3,84,84).cuda()
                        gt = select_task_pool(sinkhorn,task_ori_pool,coarse0,coarse1,MB_feats,20)
                        supp_aug, query_aug = TaskGen.Upsampling(maml,ys,yq,partial_input,gt,MB_feats,step)
                        loss_val, acc_val = maml.forward_ours(supp,ys,supp_aug,query,query_aug,yq)
                    else:
                        loss_val, acc_val = maml.forward(supp, ys, query, yq)                    
