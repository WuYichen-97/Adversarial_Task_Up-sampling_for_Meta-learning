...
class TaskGenerator():
   ...
   def Upsampling(self, maml, ys, yq, partial_input, gt, MB_feats, step):
       ...
       if self.n_shot==1:
           ...
           out = preds.reshape(32,2,3,84,84)
           # out = gt.reshape(32,5,3,84,84)
           task1 = out[:16]
           task2 = out[16:]
           
           supp1, query1 = task1[0], task1[1:].permute(1,0,2,3,4).reshape(30,3,84,84)       
           supp2, query2 = task2[0], task2[1:].permute(1,0,2,3,4).reshape(30,3,84,84)   
           ...
           loss,_,_ = self.sinkhorn(preds.reshape(2,32,-1), gt.reshape(2,32,-1))
           loss = loss + 5*loss_grad1 + 5*loss_grad2 - 5* loss_val1 - 5*loss_val2
       else:
           _, preds, _ = self.network(partial_input, MB_feats)
           out = preds.reshape(40,2,3,84,84)
           # out = gt.reshape(32,5,3,84,84)
           task1 = out[:20]
           task2 = out[20:]
           
           supp1, query1 = task1[:5].permute(1,0,2,3,4).reshape(10,3,84,84) , task1[5:].permute(1,0,2,3,4).reshape(30,3,84,84)       
           supp2, query2 = task2[:5].permute(1,0,2,3,4).reshape(10,3,84,84) , task2[5:].permute(1,0,2,3,4).reshape(30,3,84,84)   
           ...
           loss,_,_ = self.sinkhorn(preds.reshape(2,40,-1), gt.reshape(2,40,-1))
           loss = loss + 5*loss_grad1 + 5*loss_grad2 - 5* loss_val1 - 5*loss_val2 
       ...
       # if step% 1000 == 0:
       #       print('loss',step,loss)#,-20*loss_val,-10*loss_grad)#,50*s)
