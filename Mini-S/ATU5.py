...
class Encoder(nn.Module):
    ...
    def forward(self, x0, x1):
        ...
        res_v = x1.reshape(int(x1.shape[0]/15),5,3,64,84,84)#.permute(1,0,2,3,4,5) #[5,8,3,128,84,84]
        res_v = torch.mean(res_v,dim=0).reshape(15,64,84,84)
        return x,x1,v,res_v
...
class Decoder(nn.Module):
    ...
    def forward(self, x, x1, x0, v, res_v):
        ...
        noise1 = torch.tensor([-0.5]).repeat(300,1,84,84)
        noise2 = torch.tensor([0.5]).repeat(300,1,84,84)
        ...
        x_res = x_res.reshape(-1,3,3,84,84)
        ...
        s2 = F.softmax(20*s2,dim=1).reshape(-1,3,1,1,1)
...
if __name__ == "__main__":
    pcs = torch.rand(100,3,84,84)#.cuda()
    pcs1 = torch.rand(3,3,84,84)#.cuda()
