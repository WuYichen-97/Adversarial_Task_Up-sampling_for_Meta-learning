...
class DermNet(Dataset):
    ...
    def __init__(self, args, mode):
        ...
        self.mode = mode
        self.data_file = '../Dermnet_all_84.pkl'
        
        self.MB_samples_per_class = 1
        self.MB_classes = 3
        self.MB_set_size = self.MB_classes * self.MB_samples_per_class        
    ...
    def __getitem__(self, index):
        ...
        if self.args.train:
            ...
            for meta_batch_id in range(self.args.meta_batch_size):
                ...
                Memory_Bank_classes = (list(set(self.classes_idx).difference(self.choose_classes)))     
                self.Memory_Bank_classes = np.random.choice(Memory_Bank_classes,size=self.MB_classes,replace=False)
                ...
                for j in range(self.MB_classes):
                    self.samples_idx_mb = np.arange(self.data[self.Memory_Bank_classes[j]].shape[0])
                    np.random.shuffle(self.samples_idx_mb)
                    choose_samples_mb = self.samples_idx_mb[:self.MB_samples_per_class]                       
...
class ISIC(Dataset):
    ...
    def __init__(self, args, mode):
        ...
        self.mode = mode
        if mode == 'train':
            self.data_file = '../ISIC_YAO/ISIC_train.pkl'#.format(args.datadir)
        elif mode == 'test':
            self.data_file = '../ISIC_YAO/ISIC_test.pkl'#.format(args.datadir)
        ...
        for eachkey in self.data.keys():
            self.data[eachkey] = torch.tensor(np.transpose(self.data[eachkey] / np.float32(255), (0, 3, 1, 2)))
        self.MB_samples_per_class = 1
        self.MB_set_size = 2 * self.MB_samples_per_class
    ...
    def __getitem__(self, index):
        ...
        if self.args.train:
            ...
            for meta_batch_id in range(self.args.meta_batch_size):
                self.choose_classes = np.random.choice(self.classes_idx, size=self.nb_classes, replace=False)
                self.Memory_Bank_classes = (list(set(self.classes_idx).difference(self.choose_classes))) # MB different class with meta-train
                ...
                for j in range(self.nb_classes):
                    ...
                    # print('self',self.choose_classes[j],choose_samples[:self.k_shot])
                    # print(self.data[0].shape)
                    # print('self.data',self.data[0,[84,21]])
                    support_x[meta_batch_id][j * self.k_shot:(j + 1) * self.k_shot] = self.data[
                        self.choose_classes[
                            j]][choose_samples[
                                :self.k_shot], ...]
            ...
            support_y = np.zeros([self.set_size])
            self.choose_classes = np.random.choice(self.classes_idx, size=self.nb_classes, replace=False)
            query_size_test = self.data[self.choose_classes[0]].shape[0] + self.data[self.choose_classes[1]].shape[0] - self.set_size
