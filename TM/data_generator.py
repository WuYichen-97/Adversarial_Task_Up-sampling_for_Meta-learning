...
class TM(Dataset):
    ...
    def __getitem__(self, index):
        if self.args.train:
            ...
            MB_x = np.zeros((self.args.meta_batch_size, self.MB_set_size, 2866))
            ...
            for meta_batch_id in range(self.args.meta_batch_size):
                ...
                np.random.shuffle(Memory_Bank_classes)
                self.Memory_Bank_classes = Memory_Bank_classes[:self.MB_classes]#np.random.choice(Memory_Bank_classes,size=self.MB_classes,replace=False)
            ...
            return torch.FloatTensor(support_x), torch.LongTensor(support_y), torch.FloatTensor(query_x), torch.LongTensor(query_y), torch.FloatTensor(MB_x)    
        else :
            support_x = np.zeros((self.args.meta_batch_size, self.set_size, 2866))
            query_x = np.zeros((self.args.meta_batch_size, self.query_size, 2866))
